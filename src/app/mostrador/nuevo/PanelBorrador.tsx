...
import { SUCURSALES_ENTREGA, type PedidoDetalle } from "@/lib/mostrador/tipos";
import { AgregarSinVico } from "./AgregarSinVico";
import type { ResultadoAccion } from "./NuevoPedido";

// Columna "Pedido en captura": la tarjeta del borrador tal como lo tiene IA
// (partidas, totales con IVA incluido), las observaciones, y las dos salidas:
// enviar (ámbar, la única acción principal del panel) o cancelar. Debajo, el
// buscador manual para agregar piezas sin pasar por Vico.
...
export function PanelBorrador({
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              <li key={p.id} className="flex items-start gap-2 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="num-tab mt-0.5 font-mono text-xs text-tinta-suave">
                    {p.codigo ?? (p.idPiezaUsada !== null ? `Usada #${p.idPiezaUsada}` : "")}
                    {" · "}
                    {p.cantidad} × {pesos(p.precioUnitario)} IVA incluido
                  </p>
                </div>
      ...
      <AgregarSinVico idCliente={borrador?.idCliente ?? null} ocupado={bloqueado} onAgregar={onAgregar} />
    </div>
  );
}
